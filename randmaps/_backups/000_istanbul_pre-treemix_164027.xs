...
void main(void)
{
	...
	int oneTree = rmCreateObjectDef("one tree");
	rmAddObjectDefItem(oneTree, hinterTree, 4, 5.0);
	rmSetObjectDefMinDistance(oneTree, 0.0);
	rmSetObjectDefMaxDistance(oneTree, rmXTilesToFraction(treeJitter));
	// no tree-from-tree spacing: they may sit on top of each other, which
	// is what stops the placement snapping back into tidy rows
	...
	for (tr = 0; < treeRows)
	{
		...
		for (tc = 0; < treeCols)
		{
			rmPlaceObjectDefAtLoc(oneTree, 0, tx, tz, 1);
			tx = tx + treeStepX;
		}
		tz = tz + treeStepZ;
	}
