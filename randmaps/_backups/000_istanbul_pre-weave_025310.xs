...
// Which house type ended up in each city cell (-1 = none yet), plus the grid
// pitch, so we can tell whether two cells are neighbours. Positions are read
// from gCityLocs itself, so this stays correct after the zones are shuffled.
int   gCityHouse   = -1;
int   gHouseBlocks = -1;
float gPitchX      = 0.0;
float gPitchZ      = 0.0;
...
// true if an identical house block already sits in a cell orthogonally next
// to this one (one grid step away in x or in z).
bool sameHouseNextDoor(int idx = -1, int candidate = -1, int startIndex = -1, int endIndex = -1)
{
	vector here = xsArrayGetVector(gCityLocs, idx);
	for (k = startIndex; <= endIndex)
	{
		if (xsArrayGetInt(gCityHouse, k) != candidate) continue;
		vector other = xsArrayGetVector(gCityLocs, k);
		float dx = xsVectorGetX(other) - xsVectorGetX(here);
		float dz = xsVectorGetZ(other) - xsVectorGetZ(here);
		if (dx < 0.0) { dx = 0.0 - dx; }
		if (dz < 0.0) { dz = 0.0 - dz; }
		if (dz < gPitchTol)
		{
			if (dx < gPitchX + gPitchTol)
			{
				if (dx > gPitchX - gPitchTol) return true;
			}
		}
		if (dx < gPitchTol)
		{
			if (dz < gPitchZ + gPitchTol)
			{
				if (dz > gPitchZ - gPitchTol) return true;
			}
		}
	}
	return false;
...
// Fill every free cell of a range with houses, never putting the same type
// beside itself. Tries the 6 types from a random starting point and takes the
// first that has no identical neighbour.
void fillHouses(int startIndex = -1, int endIndex = -1)
{
	for (i = startIndex; <= endIndex)
	{
		bool taken = xsArrayGetBool(gCityLocsStatus, i);
		...
		int offset = rmRandInt(0, total - 1);
		int chosen = -1;

		for (t = 0; < total)
		{
			int ix = offset + t;
			if (ix >= total) { ix = ix - total; }
			int cand = xsArrayGetInt(gHouseBlocks, ix);
			if (sameHouseNextDoor(i, cand, startIndex, endIndex) == false)
			{
				chosen = cand;
				break;
			}
		}
		if (chosen < 0) { chosen = xsArrayGetInt(gHouseBlocks, offset); }

		vector loc = xsArrayGetVector(gCityLocs, i);
		rmPlaceGroupingAtLoc(chosen, 0, xsVectorGetX(loc), xsVectorGetZ(loc));
		xsArraySetBool(gCityLocsStatus, i, true);
		xsArraySetInt(gCityHouse, i, chosen);
	}
}
...
void main(void)
{
	...
	placeShoreIsland(trade5,
		...
		rmZMetersToFraction(xsVectorGetZ(flank2LocN)) + rmZMetersToFraction(h5GroupOffZ));
	...
	placeShoreIsland(trade6,
		...
		rmZMetersToFraction(xsVectorGetZ(flank2LocS)) + rmZMetersToFraction(h6GroupOffZ));
	...
	placeShoreIsland(trade3,
		...
		rmZMetersToFraction(xsVectorGetZ(flankLocN)) + rmZMetersToFraction(h3GroupOffZ));
	...
	placeShoreIsland(trade4,
		...
		rmZMetersToFraction(xsVectorGetZ(flankLocS)) + rmZMetersToFraction(h4GroupOffZ));
	...
	placeShoreIsland(tradeS, tradeSX, tradeSZ);
	...
	// The z1 gap the fort does NOT take is a normal pool cell, so a house
	// fills it instead of leaving a hole on the waterfront.
	...
	gCityHouse      = xsArrayCreateInt(NUM_CITY_LOCS, -1, "house type per cell");
	// grid pitch, for the "no identical neighbour" test
	gPitchX   = colPitch;
	gPitchZ   = rowPitch;
	gPitchTol = rmXTilesToFraction(4);

	// north centre: the free z1 gap, then z2/z3 minus the menagerie cell
	xsArraySetVector(gCityLocs,  0, xsVectorSet(nx4, 0.0, nz1));
	xsArraySetVector(gCityLocs,  1, xsVectorSet(nx1, 0.0, nz2));
	xsArraySetVector(gCityLocs,  2, xsVectorSet(nx2, 0.0, nz2));
	xsArraySetVector(gCityLocs,  3, xsVectorSet(nx5, 0.0, nz2));
	xsArraySetVector(gCityLocs,  4, xsVectorSet(nx1, 0.0, nz3));
	xsArraySetVector(gCityLocs,  5, xsVectorSet(nx2, 0.0, nz3));
	xsArraySetVector(gCityLocs,  6, xsVectorSet(nx4, 0.0, nz3));
	xsArraySetVector(gCityLocs,  7, xsVectorSet(nx5, 0.0, nz3));
	// north suburb: z4 minus the factory corner, z5 minus both corners
	xsArraySetVector(gCityLocs,  8, xsVectorSet(nx1, 0.0, nz4));
	xsArraySetVector(gCityLocs,  9, xsVectorSet(nx2, 0.0, nz4));
	xsArraySetVector(gCityLocs, 10, xsVectorSet(nx3, 0.0, nz4));
	xsArraySetVector(gCityLocs, 11, xsVectorSet(nx4, 0.0, nz4));
	xsArraySetVector(gCityLocs, 12, xsVectorSet(nx2, 0.0, nz5));
	xsArraySetVector(gCityLocs, 13, xsVectorSet(nx3, 0.0, nz5));
	xsArraySetVector(gCityLocs, 14, xsVectorSet(nx4, 0.0, nz5));
	// south centre: mirrored
	xsArraySetVector(gCityLocs, 15, xsVectorSet(sx2, 0.0, sz1));
	xsArraySetVector(gCityLocs, 16, xsVectorSet(sx1, 0.0, sz2));
	xsArraySetVector(gCityLocs, 17, xsVectorSet(sx4, 0.0, sz2));
	xsArraySetVector(gCityLocs, 18, xsVectorSet(sx5, 0.0, sz2));
	xsArraySetVector(gCityLocs, 19, xsVectorSet(sx1, 0.0, sz3));
	xsArraySetVector(gCityLocs, 20, xsVectorSet(sx2, 0.0, sz3));
	xsArraySetVector(gCityLocs, 21, xsVectorSet(sx4, 0.0, sz3));
	xsArraySetVector(gCityLocs, 22, xsVectorSet(sx5, 0.0, sz3));
	// south suburb: same exclusions, mirrored
	xsArraySetVector(gCityLocs, 23, xsVectorSet(sx2, 0.0, sz4));
	xsArraySetVector(gCityLocs, 24, xsVectorSet(sx3, 0.0, sz4));
	xsArraySetVector(gCityLocs, 25, xsVectorSet(sx4, 0.0, sz4));
	xsArraySetVector(gCityLocs, 26, xsVectorSet(sx5, 0.0, sz4));
	xsArraySetVector(gCityLocs, 27, xsVectorSet(sx2, 0.0, sz5));
	xsArraySetVector(gCityLocs, 28, xsVectorSet(sx3, 0.0, sz5));
	xsArraySetVector(gCityLocs, 29, xsVectorSet(sx4, 0.0, sz5));

	shuffle(gCityLocs, N_CENTRE_START, N_CENTRE_END);
	shuffle(gCityLocs, N_SUBURB_START, N_SUBURB_END);
	shuffle(gCityLocs, S_CENTRE_START, S_CENTRE_END);
	shuffle(gCityLocs, S_SUBURB_START, S_SUBURB_END);
	...
	fillHouses(N_CENTRE_START, N_SUBURB_END);
	fillHouses(S_CENTRE_START, S_SUBURB_END);
