...
void main(void)
{
	...
	// --- WILD FLANK CLIFFS - zp_zaustralia.xs big-island idiom, UNBOXED ---
	// Shape = traced outline + interior influence points + coherence 0.8.
	// Guards are CONSTRAINTS, not fences:
	//   wildAvoidLane   : 10 m off both trade routes (Australia's own value)
	//   wildAvoidPlateau: 4 m off every plateau already built - the cities,
	//                     hinterlands, gun islands and the balance cliff.
	//                     THIS is what makes touching the city impossible.
	int wildAvoidLane = rmCreateTradeRouteDistanceConstraint("wild flank off lanes", 10.0);
	// 0.0 = flush contact, same semantic as the hinterland vs the street:
	// the mass grows right up to the city plateau and stops - no moat,
	// no overlap. The cliff wall meets the promenade directly.
	int wildAvoidPlateau = rmCreateClassDistanceConstraint("wild flank off plateaus",
		rmClassID("classPlateau"), 0.0);

	// WEST: one solid high cliff mass, walls into the sea
	int wildW = rmCreateArea("west wild cliff");
	rmSetAreaWarnFailure(wildW, false);
	// ask LESS than the outline encloses (5067 tiles) - Australia's rule.
	// The fill hugs the traced skeleton and coherence shapes the ragged edge;
	// there is NO box, so the outline is the shape, not a fence.
	rmSetAreaSize(wildW, 0.05, 0.05);
	// 0.8 = Tasmania's ragged coast-mountains; lower = wilder
	// seed INSIDE the outline - without a location the seed is random and
	// the mass sprouts anywhere on the map (every shipped example sets one)
	rmSetAreaLocation(wildW, 0.24, 0.72);
	rmSetAreaCoherence(wildW, 0.8);
	// same height as the south balance cliff - equal player conditions
	rmSetAreaBaseHeight(wildW, cityHeight + balanceRise);
	rmSetAreaSmoothDistance(wildW, 20);
	rmSetAreaMix(wildW, hinterMix);
	rmSetAreaCliffType(wildW, hinterCliff);
	rmSetAreaCliffEdge(wildW, 1, 1.0, 0.1, 1.0, 0);
	rmSetAreaCliffHeight(wildW, 0, 0.0, 1.0);
	rmAddAreaToClass(wildW, rmClassID("classCliff"));
	rmAddAreaConstraint(wildW, wildAvoidLane);
	rmAddAreaConstraint(wildW, wildAvoidPlateau);
	rmSetAreaObeyWorldCircleConstraint(wildW, false);
	// the traced outline (Australia idiom: chain of short segments)
	rmAddAreaInfluenceSegment(wildW, 0.34, 0.62, 0.34, 0.74);
	rmAddAreaInfluenceSegment(wildW, 0.34, 0.78, 0.32, 0.86);
	rmAddAreaInfluenceSegment(wildW, 0.30, 0.90, 0.26, 0.90);
	rmAddAreaInfluenceSegment(wildW, 0.22, 0.86, 0.16, 0.80);
	rmAddAreaInfluenceSegment(wildW, 0.12, 0.74, 0.10, 0.66);
	rmAddAreaInfluenceSegment(wildW, 0.12, 0.62, 0.16, 0.60);
	rmAddAreaInfluenceSegment(wildW, 0.20, 0.58, 0.26, 0.58);
	rmAddAreaInfluenceSegment(wildW, 0.30, 0.60, 0.33, 0.61);
	// interior influence points - extra lobes, extra wildness
	rmAddAreaInfluencePoint(wildW, 0.28, 0.80);
	rmAddAreaInfluencePoint(wildW, 0.18, 0.70);
	rmAddAreaInfluencePoint(wildW, 0.24, 0.62);
	rmAddAreaInfluencePoint(wildW, 0.31, 0.72);
	rmBuildArea(wildW);

	// EAST: the 180-degree mirror
	int wildE = rmCreateArea("east wild cliff");
	rmSetAreaWarnFailure(wildE, false);
	// ask LESS than the outline encloses (5067 tiles) - Australia's rule.
	// The fill hugs the traced skeleton and coherence shapes the ragged edge;
	// there is NO box, so the outline is the shape, not a fence.
	rmSetAreaSize(wildE, 0.05, 0.05);
	// 0.8 = Tasmania's ragged coast-mountains; lower = wilder
	// seed INSIDE the outline - without a location the seed is random and
	// the mass sprouts anywhere on the map (every shipped example sets one)
	rmSetAreaLocation(wildE, 0.76, 0.28);
	rmSetAreaCoherence(wildE, 0.8);
	// same height as the south balance cliff - equal player conditions
	rmSetAreaBaseHeight(wildE, cityHeight + balanceRise);
	rmSetAreaSmoothDistance(wildE, 20);
	rmSetAreaMix(wildE, hinterMix);
	rmSetAreaCliffType(wildE, hinterCliff);
	rmSetAreaCliffEdge(wildE, 1, 1.0, 0.1, 1.0, 0);
	rmSetAreaCliffHeight(wildE, 0, 0.0, 1.0);
	rmAddAreaToClass(wildE, rmClassID("classCliff"));
	rmAddAreaConstraint(wildE, wildAvoidLane);
	rmAddAreaConstraint(wildE, wildAvoidPlateau);
	rmSetAreaObeyWorldCircleConstraint(wildE, false);
	// the traced outline (Australia idiom: chain of short segments)
	rmAddAreaInfluenceSegment(wildE, 0.66, 0.38, 0.66, 0.26);
	rmAddAreaInfluenceSegment(wildE, 0.66, 0.22, 0.68, 0.14);
	rmAddAreaInfluenceSegment(wildE, 0.70, 0.10, 0.74, 0.10);
	rmAddAreaInfluenceSegment(wildE, 0.78, 0.14, 0.84, 0.20);
	rmAddAreaInfluenceSegment(wildE, 0.88, 0.26, 0.90, 0.34);
	rmAddAreaInfluenceSegment(wildE, 0.88, 0.38, 0.84, 0.40);
	rmAddAreaInfluenceSegment(wildE, 0.80, 0.42, 0.74, 0.42);
	rmAddAreaInfluenceSegment(wildE, 0.70, 0.40, 0.67, 0.39);
	// interior influence points - extra lobes, extra wildness
	rmAddAreaInfluencePoint(wildE, 0.72, 0.20);
	rmAddAreaInfluencePoint(wildE, 0.82, 0.30);
	rmAddAreaInfluencePoint(wildE, 0.76, 0.38);
	rmAddAreaInfluencePoint(wildE, 0.69, 0.28);
	rmBuildArea(wildE);
