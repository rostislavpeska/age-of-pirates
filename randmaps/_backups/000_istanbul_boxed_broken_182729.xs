...
void main(void)
{
	...
	// WEST: the whole wild mass IS the cliff - walls rise from the water,
	// same construction as the city plateaus (cliff area beside water)
	int wildW = rmCreateArea("west wild land");
	rmSetAreaWarnFailure(wildW, false);
	// over-ask on purpose: feasible region is ~13k tiles, asking 16.2k
	// fills every reachable cell - no water pockets inside the mass
	rmSetAreaSize(wildW, 0.18, 0.18);
	// 0.8 = Tasmania's ragged coast-mountains; lower = wilder coastline
	...
	// EAST: the mirror - one solid high cliff mass
	int wildE = rmCreateArea("east wild land");
	rmSetAreaWarnFailure(wildE, false);
	// over-ask on purpose: feasible region is ~13k tiles, asking 16.2k
	// fills every reachable cell - no water pockets inside the mass
	rmSetAreaSize(wildE, 0.18, 0.18);
	// 0.8 = Tasmania's ragged coast-mountains; lower = wilder coastline
	rmSetAreaCoherence(wildE, 0.3);
	// same height as the south balance cliff - equal player conditions
