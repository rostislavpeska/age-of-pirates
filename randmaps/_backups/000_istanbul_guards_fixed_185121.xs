...
void main(void)
{
	...
	// 0.0 = flush: cliffs grow right up to the city plateau and stop
	int wildAvoidPlateau = rmCreateClassDistanceConstraint("cliffs off plateaus",
	...
	rmSetAreaLocation(wildW, 0.20, 0.70);
	rmSetAreaBaseHeight(wildW, cityHeight + balanceRise);
	rmSetAreaSmoothDistance(wildW, 10);
	...
	rmAddAreaConstraint(wildW, wildAvoidPlateau);   // flush against the city
	...
	rmSetAreaLocation(wildE, 0.80, 0.30);
	rmSetAreaBaseHeight(wildE, cityHeight + balanceRise);
	rmSetAreaSmoothDistance(wildE, 10);
