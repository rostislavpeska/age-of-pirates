...
void main(void)
{
	...
	int   northFlankDist  = 3;    // out from the north island's NE coast
	int   northFlankAlong = 0;    // along that coast
	...
	float nBendX = nx5 + flankProm + rmXTilesToFraction(northFlankDist + 9);
	float sBendX = sx1 - flankProm - rmXTilesToFraction(southFlankDist + 9);
	...
	rmAddTradeRouteWaypoint(tradeRouteN, 1.0, nz3Ask);
	rmAddTradeRouteWaypoint(tradeRouteN, nBendX, nz3Ask);
	...
	rmAddTradeRouteWaypoint(tradeRouteS, 0.0, sz3Ask);
	rmAddTradeRouteWaypoint(tradeRouteS, sBendX, sz3Ask);
	...
	placeShoreIsland(trade3, nFlankX, nFlankZ);   // north island, NE coast
	placeShoreIsland(trade4, sFlankX, sFlankZ);   // south island, SW coast
	...
	placeShoreIsland(tradeN,
		rmXMetersToFraction(xsVectorGetX(harbourLocN)) - rmXMetersToFraction(northHarbourOffX),
		rmZMetersToFraction(xsVectorGetZ(harbourLocN)) - rmZMetersToFraction(northHarbourOffZ));
