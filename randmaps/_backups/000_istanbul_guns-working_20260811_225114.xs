...
int gScaffoldIdx = 0;
void gunScaffold(float x = 0.0, float z = 0.0)
{
...
void gunRiver(float x = 0.0, float z = 0.0)
{
...
void placeGun(int grouping = -1, float x = 0.0, float z = 0.0)
{
...
void main(void)
{
	...
	float nRouteAsk   = 0.46;   // north shipping lane, runs W -> E
	float sRouteAsk   = 0.54;   // south shipping lane, runs E -> W
	...
	int   southCityDist   = 24;   // south lane -> south city, first block row
	...
	int   northCityShiftX = -10;  // north city, 10 tiles west
	int   southCityShiftX =  0;   // south city, centred
	...
	//   _01 -> orientz -1, muzzle points NORTH -> belongs SOUTH of a route
	//   _02 -> orientz +1, muzzle points SOUTH -> belongs NORTH of a route
	string gunFacingNorth = "IS_Shore_Fixed_Gun_01";
	string gunFacingSouth = "IS_Shore_Fixed_Gun_02";
	...
	// ========================================================================
	gunScaffold(nx1, nRouteAsk - rmZTilesToFraction(northGunDist));
	gunScaffold(nx5, nRouteAsk - rmZTilesToFraction(northGunDist));
	gunScaffold(sx1, sRouteAsk + rmZTilesToFraction(southGunDist));
	gunScaffold(sx5, sRouteAsk + rmZTilesToFraction(southGunDist));
	...
	// ========================================================================
	int gunNW = rmCreateGrouping("gun nw", gunFacingSouth);
	int gunNE = rmCreateGrouping("gun ne", gunFacingSouth);
	int gunSW = rmCreateGrouping("gun sw", gunFacingNorth);
	int gunSE = rmCreateGrouping("gun se", gunFacingNorth);
	...
	placeGun(gunNW, nx1, nGunZ);
	placeGun(gunNE, nx5, nGunZ);
	placeGun(gunSW, sx1, sGunZ);
	placeGun(gunSE, sx5, sGunZ);
	...
	int cityNbox = rmCreateBoxConstraint("north city box",
		nx1 - margin, nz5 - margin, nx5 + margin, nz1 + margin, 0.01);
	...
	int citySbox = rmCreateBoxConstraint("south city box",
		sx1 - margin, sz1 - margin, sx5 + margin, sz5 + margin, 0.01);
