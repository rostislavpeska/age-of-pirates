...
void main(void)
{
	...
	// ---- FLANK CLIFFS: the two green areas, outside the lanes -----------
	// x 0.0 west -> 1.0 east,  z 0.0 south -> 1.0 north.
	// The lanes run straight up/down at x 0.26 (west) and x 0.70 (east),
	// so these sit beyond them and cannot block the water.
	float westCliffX      = 0.13;
	float westCliffZ      = 0.50;
	float eastCliffX      = 0.87;
	float eastCliffZ      = 0.50;
	float flankCliffSize  = 0.03;  // how big
	float flankCliffRise  = 14.0;  // how high
	int   flankCliffLand  = 2600;  // land laid under it, in tiles
	...
	// the two flank cliffs. They stand in open water, so each lays its own
	// land first - that is the third argument.
	cliffMass(westCliffX, westCliffZ, flankCliffLand, flankCliffSize, flankCliffRise, 0.0);
	cliffMass(eastCliffX, eastCliffZ, flankCliffLand, flankCliffSize, flankCliffRise, 0.0);
