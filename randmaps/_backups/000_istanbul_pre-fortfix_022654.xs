...
void main(void)
{
	...
	// menagerie beside the mosque, the natives and fort on z1, the factory on
	// z4, and the two construction blocks on the z5 corners.
	const int N_CENTRE_START = 0;    const int N_CENTRE_END = 6;
	const int N_SUBURB_START = 7;    const int N_SUBURB_END = 13;
	const int S_CENTRE_START = 14;   const int S_CENTRE_END = 20;
	const int S_SUBURB_START = 21;   const int S_SUBURB_END = 27;
	const int NUM_CITY_LOCS  = 28;
	...
	// north centre: z2 and z3, minus nx4/nz2 which the menagerie owns
	xsArraySetVector(gCityLocs,  0, xsVectorSet(nx1, 0.0, nz2));
	xsArraySetVector(gCityLocs,  1, xsVectorSet(nx2, 0.0, nz2));
	xsArraySetVector(gCityLocs,  2, xsVectorSet(nx5, 0.0, nz2));
	xsArraySetVector(gCityLocs,  3, xsVectorSet(nx1, 0.0, nz3));
	xsArraySetVector(gCityLocs,  4, xsVectorSet(nx2, 0.0, nz3));
	xsArraySetVector(gCityLocs,  5, xsVectorSet(nx4, 0.0, nz3));
	xsArraySetVector(gCityLocs,  6, xsVectorSet(nx5, 0.0, nz3));
	// north suburb: z4 minus the factory corner, z5 minus both corners
	xsArraySetVector(gCityLocs,  7, xsVectorSet(nx1, 0.0, nz4));
	xsArraySetVector(gCityLocs,  8, xsVectorSet(nx2, 0.0, nz4));
	xsArraySetVector(gCityLocs,  9, xsVectorSet(nx3, 0.0, nz4));
	xsArraySetVector(gCityLocs, 10, xsVectorSet(nx4, 0.0, nz4));
	xsArraySetVector(gCityLocs, 11, xsVectorSet(nx2, 0.0, nz5));
	xsArraySetVector(gCityLocs, 12, xsVectorSet(nx3, 0.0, nz5));
	xsArraySetVector(gCityLocs, 13, xsVectorSet(nx4, 0.0, nz5));
	// south centre: mirrored, minus sx2/sz2 for the menagerie
	xsArraySetVector(gCityLocs, 14, xsVectorSet(sx1, 0.0, sz2));
	xsArraySetVector(gCityLocs, 15, xsVectorSet(sx4, 0.0, sz2));
	xsArraySetVector(gCityLocs, 16, xsVectorSet(sx5, 0.0, sz2));
	xsArraySetVector(gCityLocs, 17, xsVectorSet(sx1, 0.0, sz3));
	xsArraySetVector(gCityLocs, 18, xsVectorSet(sx2, 0.0, sz3));
	xsArraySetVector(gCityLocs, 19, xsVectorSet(sx4, 0.0, sz3));
	xsArraySetVector(gCityLocs, 20, xsVectorSet(sx5, 0.0, sz3));
	// south suburb: same exclusions, mirrored
	xsArraySetVector(gCityLocs, 21, xsVectorSet(sx2, 0.0, sz4));
	xsArraySetVector(gCityLocs, 22, xsVectorSet(sx3, 0.0, sz4));
	xsArraySetVector(gCityLocs, 23, xsVectorSet(sx4, 0.0, sz4));
	xsArraySetVector(gCityLocs, 24, xsVectorSet(sx5, 0.0, sz4));
	xsArraySetVector(gCityLocs, 25, xsVectorSet(sx2, 0.0, sz5));
	xsArraySetVector(gCityLocs, 26, xsVectorSet(sx3, 0.0, sz5));
	xsArraySetVector(gCityLocs, 27, xsVectorSet(sx4, 0.0, sz5));
	...
	rmPlaceGroupingAtLoc(menagerieN, 0, nx4, nz2);
	...
	rmPlaceGroupingAtLoc(menagerieS, 0, sx2, sz2);
	...
	while(true)
	{
		int northHouse = xsArrayGetInt(houseGroupings, rmRandInt(0, xsArrayGetSize(houseGroupings) - 1));
		if (filler(northHouse, N_CENTRE_START, N_SUBURB_END) == false)
		{
			break;
		}
	}
	...
	while(true)
	{
		int southHouse = xsArrayGetInt(houseGroupings, rmRandInt(0, xsArrayGetSize(houseGroupings) - 1));
		if (filler(southHouse, S_CENTRE_START, S_SUBURB_END) == false)
		{
			break;
		}
	}
