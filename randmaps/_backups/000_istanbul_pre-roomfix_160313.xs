...
void main(void)
{
	...
	string hinterForest   = "z42 Italian Forest";// forestType1 on Black Sea, the
	                                             // one its SOUTHERN forests use
	...
	// --- how much room each peninsula actually got ------------------------
	// From its seam out to its own map edge, capped by the hinterland box.
	// Measured, not assumed: the 90-tile box reach is far more than the map
	// actually has behind row 6, so anything sized or placed off hinterDeep
	// lands beyond the world edge and silently fails to build.
	...
	// Same box, location and size as the grass it stands on, so the trees fill
	// that footprint and nothing else. The forest settings are Black Sea's
	// southForest block verbatim (density 1.0, clumpiness 0.0, underbrush 0.0);
	// avoidStreet is the constraint the grass already uses, so the wood stops
	// at the cobbles for the same reason the grass does.
	int forestN = rmCreateArea("forestNorth");
	rmSetAreaWarnFailure(forestN, true);
	rmSetAreaSize(forestN, rmAreaTilesToFraction(1500), rmAreaTilesToFraction(1500));
	rmSetAreaLocation(forestN, nx3, hinterEdgeN + northRoom * 0.5);
	rmAddAreaInfluenceSegment(forestN, nx1, hinterEdgeN + northRoom * 0.5,
		nx5, hinterEdgeN + northRoom * 0.5);
	...
	rmSetAreaSize(forestS, rmAreaTilesToFraction(1500), rmAreaTilesToFraction(1500));
	rmSetAreaLocation(forestS, sx3, hinterEdgeS - southRoom * 0.5);
	rmAddAreaInfluenceSegment(forestS, sx1, hinterEdgeS - southRoom * 0.5,
		sx5, hinterEdgeS - southRoom * 0.5);
