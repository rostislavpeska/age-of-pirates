...
void main(void)
{
	...
	int   treeClumps      = 25;   // placements per island
	int   treesPerClump   = 100;  // trees IN each placement. This is the
	                              // multiplier - rmAddObjectDefItem's 3rd
	                              // argument, exactly as TreeAmazon uses it
	                              // (20 trees, 10 m cluster) in shipped maps.
	float treeClumpSpread = 100.0;// metres each clump spreads
	int   treeSpread      = 70;   // tiles the clumps scatter from the centre
	...
	rmPlaceObjectDefAtLoc(treesN, 0, forestNX, forestNZ, treeClumps);
	...
	rmPlaceObjectDefAtLoc(treesS, 0, forestSX, forestSZ, treeClumps);
