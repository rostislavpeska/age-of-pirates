...
void main(void)
{
	...
	// ---- THE FORT: canal row too, in one of the two gaps between a native
	// and the park (x2 or x4). Rolled per island; the other gap stays open.
	...
	int flipFortN = rmRandInt(0, 100);
	int flipFortS = rmRandInt(0, 100);

	float fortNX = nx4;   if (flipFortN < 50) { fortNX = nx2; }
	float fortSX = sx4;   if (flipFortS < 50) { fortSX = sx2; }

	rmEchoInfo("fort: flipFortN=" + flipFortN + " flipFortS=" + flipFortS + " (<50 = west gap)");
	rmPlaceGroupingAtLoc(fortN, 0, fortNX, nz1);
	rmPlaceGroupingAtLoc(fortS, 0, fortSX, sz1);
	...
	// Fixed cells are NOT in here: the x3 spine (park/mosque/bazaar), the
	// natives and fort on z1, and the factory corner on z4.
	const int N_CENTRE_START = 0;    const int N_CENTRE_END = 7;
	const int N_SUBURB_START = 8;    const int N_SUBURB_END = 16;
	const int S_CENTRE_START = 17;   const int S_CENTRE_END = 24;
	const int S_SUBURB_START = 25;   const int S_SUBURB_END = 33;
	const int NUM_CITY_LOCS  = 34;
	...
	// north centre: rows z2 and z3, both sides of the spine
	xsArraySetVector(gCityLocs,  0, xsVectorSet(nx1, 0.0, nz2));
	xsArraySetVector(gCityLocs,  1, xsVectorSet(nx2, 0.0, nz2));
	xsArraySetVector(gCityLocs,  2, xsVectorSet(nx4, 0.0, nz2));
	...
	// north suburb: rows z4 and z5  (z4/x5 is the factory, left out)
	...
	xsArraySetVector(gCityLocs, 12, xsVectorSet(nx1, 0.0, nz5));
	xsArraySetVector(gCityLocs, 13, xsVectorSet(nx2, 0.0, nz5));
	xsArraySetVector(gCityLocs, 14, xsVectorSet(nx3, 0.0, nz5));
	xsArraySetVector(gCityLocs, 15, xsVectorSet(nx4, 0.0, nz5));
	xsArraySetVector(gCityLocs, 16, xsVectorSet(nx5, 0.0, nz5));
	// south centre
	xsArraySetVector(gCityLocs, 17, xsVectorSet(sx1, 0.0, sz2));
	xsArraySetVector(gCityLocs, 18, xsVectorSet(sx2, 0.0, sz2));
	xsArraySetVector(gCityLocs, 19, xsVectorSet(sx4, 0.0, sz2));
	xsArraySetVector(gCityLocs, 20, xsVectorSet(sx5, 0.0, sz2));
	xsArraySetVector(gCityLocs, 21, xsVectorSet(sx1, 0.0, sz3));
	xsArraySetVector(gCityLocs, 22, xsVectorSet(sx2, 0.0, sz3));
	xsArraySetVector(gCityLocs, 23, xsVectorSet(sx4, 0.0, sz3));
	xsArraySetVector(gCityLocs, 24, xsVectorSet(sx5, 0.0, sz3));
	// south suburb  (z4/x1 is the factory, left out)
	xsArraySetVector(gCityLocs, 25, xsVectorSet(sx2, 0.0, sz4));
	xsArraySetVector(gCityLocs, 26, xsVectorSet(sx3, 0.0, sz4));
	xsArraySetVector(gCityLocs, 27, xsVectorSet(sx4, 0.0, sz4));
	xsArraySetVector(gCityLocs, 28, xsVectorSet(sx5, 0.0, sz4));
	xsArraySetVector(gCityLocs, 29, xsVectorSet(sx1, 0.0, sz5));
	xsArraySetVector(gCityLocs, 30, xsVectorSet(sx2, 0.0, sz5));
	xsArraySetVector(gCityLocs, 31, xsVectorSet(sx3, 0.0, sz5));
	xsArraySetVector(gCityLocs, 32, xsVectorSet(sx4, 0.0, sz5));
	xsArraySetVector(gCityLocs, 33, xsVectorSet(sx5, 0.0, sz5));
	...
	int nCentreBlocks = xsArrayCreateInt(3, -1, "north centre blocks");
	xsArraySetInt(nCentreBlocks, 0, bankN);
	xsArraySetInt(nCentreBlocks, 1, embassyN);
	xsArraySetInt(nCentreBlocks, 2, menagerieN);
	placeGroupings(nCentreBlocks, N_CENTRE_START);

	int sCentreBlocks = xsArrayCreateInt(3, -1, "south centre blocks");
	xsArraySetInt(sCentreBlocks, 0, bankS);
	xsArraySetInt(sCentreBlocks, 1, embassyS);
	xsArraySetInt(sCentreBlocks, 2, menagerieS);
