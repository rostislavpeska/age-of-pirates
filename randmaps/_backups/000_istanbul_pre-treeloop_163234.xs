...
void main(void)
{
	...
	int   treeClumps      = 60;   // placements per island
	int   treesPerClump   = 6;    // trees IN each placement. This is the
	                              // multiplier - rmAddObjectDefItem's 3rd
	                              // argument, exactly as TreeAmazon uses it
	                              // (20 trees, 10 m cluster) in shipped maps.
	float treeClumpSpread = 16.0; // metres each clump spreads. A placement
	                              // must fit ALL its trees or the whole
	                              // placement is dropped - and the
	                              // countryside is only ~6-11 tiles deep.
	int   treeSpread      = 45;   // tiles the clump centres scatter
	...
	// --- a bunch of trees on each countryside ------------------------------
	int treesN = rmCreateObjectDef("north trees");
	rmAddObjectDefItem(treesN, hinterTree, treesPerClump, treeClumpSpread);
	rmSetObjectDefMinDistance(treesN, 0.0);
	rmSetObjectDefMaxDistance(treesN, rmXTilesToFraction(treeSpread));
	rmAddObjectDefConstraint(treesN, hinterNbox);   // stay on this countryside
	rmPlaceObjectDefAtLoc(treesN, 0, nx3, hinterEdgeN + northRoom * 0.5, treeClumps);

	int treesS = rmCreateObjectDef("south trees");
	rmAddObjectDefItem(treesS, hinterTree, treesPerClump, treeClumpSpread);
	rmSetObjectDefMinDistance(treesS, 0.0);
	rmSetObjectDefMaxDistance(treesS, rmXTilesToFraction(treeSpread));
	rmAddObjectDefConstraint(treesS, hinterSbox);   // stay on this countryside
	rmPlaceObjectDefAtLoc(treesS, 0, sx3, hinterEdgeS - southRoom * 0.5, treeClumps);
