...
void main(void)
{
	...
	int forestTest = rmCreateArea("forest test");
	rmSetAreaWarnFailure(forestTest, true);
	rmSetAreaSize(forestTest, rmAreaTilesToFraction(200), rmAreaTilesToFraction(600));
	rmSetAreaLocation(forestTest, 0.15, 0.50);
	...
	rmSetAreaObeyWorldCircleConstraint(forestTest, false);
	rmBuildArea(forestTest);
	// ===== END ISOLATION TEST ==============================================

	// --- one wood on each hinterland ---------------------------------------
	// Location is hardcoded at the top of the file, not derived from anything.
	// Forest settings are Black Sea's southForest block verbatim; the box keeps
	// the wood inside its own countryside and avoidStreet keeps it off the city.
	int forestN = rmCreateArea("forestNorth");
	rmSetAreaWarnFailure(forestN, true);
	rmSetAreaSize(forestN, rmAreaTilesToFraction(200), rmAreaTilesToFraction(1500));
	rmSetAreaLocation(forestN, forestNX, forestNZ);
	...
	rmSetAreaObeyWorldCircleConstraint(forestN, false);
	rmBuildArea(forestN);

	int forestS = rmCreateArea("forestSouth");
	rmSetAreaWarnFailure(forestS, true);
	rmSetAreaSize(forestS, rmAreaTilesToFraction(200), rmAreaTilesToFraction(1500));
	rmSetAreaLocation(forestS, forestSX, forestSZ);
	...
	rmSetAreaObeyWorldCircleConstraint(forestS, false);
	rmBuildArea(forestS);
