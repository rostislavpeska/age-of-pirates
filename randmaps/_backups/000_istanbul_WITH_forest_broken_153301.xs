...
void main(void)
{
	...
	// Own loop names: main already declares "int i" further down (the 24-block
	// setup), and a bare "i" up here would implicitly declare it first, making
	// that later declaration a duplicate. Same reason the map's other main-level
	// loop uses "in".
	for (nw = 0; < hinterTrees)
	{
		int woodN = rmCreateArea("north wood " + nw);
	...
	for (sw = 0; < hinterTrees)
	{
		int woodS = rmCreateArea("south wood " + sw);
