...
// --- the cliffs: coherent blobs shaped ONLY by the constraints ---------
	...
	// ---- THE WALL: between the city and its countryside ------------------
	// Sits between street rows 3 and 4, pushed wallOutTiles past the city's
	// countryside-facing edge - it stands ON the flank cliff, which is why it
	// is an INSTANCE placement (targetable later if the cliff misbehaves).
	// NOTE: IS_Wall_SW.xml was deployed to Game/RandMaps/groupings 2026-08-12
	// 22:36 - the game must have been STARTED after that to load it.
	int   wallOutTiles = 7;   // tiles past the city edge, toward the countryside
	int wallSW = rmCreateGrouping("wall sw", "IS_Wall_SW");
	rmSetGroupingMinDistance(wallSW, 0.0);
	rmSetGroupingMaxDistance(wallSW, 0.5);
	int wallPlacementSW = rmPlaceGroupingInstanceAtLoc(wallSW,
		nx1 - margin - rmXTilesToFraction(wallOutTiles),
		(nz3 + nz4) * 0.5, 0);
