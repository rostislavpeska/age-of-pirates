...
int gScaffoldIdx = 0;
void shoreScaffold(float x = 0.0, float z = 0.0, int tiles = 300)
{
...
void shoreRiver(float x = 0.0, float z = 0.0, int width = 13, float reach = 0.05)
{
...
void main(void)
{
	...
	shoreScaffold(nx5 + flankProm + rmXTilesToFraction(northPirateDist),
		nz3Ask + rmZTilesToFraction(northPirateBack), 900);
	shoreScaffold(sx1 - flankProm - rmXTilesToFraction(southPirateDist),
		sz3Ask - rmZTilesToFraction(southPirateBack), 900);
	...
	int avoidGuardW_far = rmCreateAreaDistanceConstraint("off west guard far", guardW, 11.0);
	int avoidGuardE_far = rmCreateAreaDistanceConstraint("off east guard far", guardE, 11.0);
...
// --- the cliffs: coherent blobs shaped ONLY by the constraints ---------
	...
	// over-ask on purpose: the pocket between city, guard and rim is the
	// shape; the ask just has to be big enough to flood it
	rmSetAreaSize(wildLowW, 0.135, 0.135);
	...
	rmAddAreaToClass(wildLowW, rmClassID("classCliff"));
	// same constraint the back countryside uses: flush to the cobbles
	...
	rmSetAreaSize(wildLowE, 0.135, 0.135);
	...
	rmAddAreaToClass(wildLowE, rmClassID("classCliff"));
	...
	int   wallOutTilesN = 8;
	int   wallOffTilesN = 0;
	int   wallOutTilesS = 11;
	int   wallOffTilesS = 0;

	// ---- GRASS STRIPES UNDER THE WALLS -----------------------------------
	// The city street floods past the promenade, so each wall got cobbles
	// under it. These stripes repaint a precise band of CITY STREET cobbles
	// along each wall's axis BEFORE the wall spawns. They reuse the wall
	// knobs, so moving a wall moves its stripe with it. Paint only: no
	// height, no cliff, no class - nothing else can change.
	int   wallStripeWide = 7;   // tiles across - a narrow band
	int   wallStripeLong = 84;  // tiles along  (wall grouping is 81 tall)
	int   wallStripeInN  = 8;   // north stripe slide toward its city
	int   wallStripeInS  = 9;   // south stripe slide toward its city
	...
	int stripeW = rmCreateArea("wall stripe west");
	rmSetAreaWarnFailure(stripeW, false);
	rmSetAreaSize(stripeW,
		rmAreaTilesToFraction(wallStripeWide * wallStripeLong),
		rmAreaTilesToFraction(wallStripeWide * wallStripeLong));
	rmSetAreaCoherence(stripeW, 1.0);
	rmSetAreaLocation(stripeW, wallXN + rmXTilesToFraction(wallStripeInN), wallZN);
	rmAddAreaInfluenceSegment(stripeW,
		wallXN + rmXTilesToFraction(wallStripeInN),
		wallZN - rmZTilesToFraction(wallStripeLong / 2),
		wallXN + rmXTilesToFraction(wallStripeInN),
		wallZN + rmZTilesToFraction(wallStripeLong / 2));
	// CITY STREET terrain - same texture the city plateaus paint
	rmSetAreaTerrainType(stripeW, "city\ground1_cob");
	rmSetAreaObeyWorldCircleConstraint(stripeW, false);
	rmBuildArea(stripeW);

	int stripeE = rmCreateArea("wall stripe east");
	rmSetAreaWarnFailure(stripeE, false);
	rmSetAreaSize(stripeE,
		rmAreaTilesToFraction(wallStripeWide * wallStripeLong),
		rmAreaTilesToFraction(wallStripeWide * wallStripeLong));
	rmSetAreaCoherence(stripeE, 1.0);
	rmSetAreaLocation(stripeE, wallXS - rmXTilesToFraction(wallStripeInS), wallZS);
	rmAddAreaInfluenceSegment(stripeE,
		wallXS - rmXTilesToFraction(wallStripeInS),
		wallZS - rmZTilesToFraction(wallStripeLong / 2),
		wallXS - rmXTilesToFraction(wallStripeInS),
		wallZS + rmZTilesToFraction(wallStripeLong / 2));
	// CITY STREET terrain - same texture the city plateaus paint
	rmSetAreaTerrainType(stripeE, "city\ground1_cob");
	rmSetAreaObeyWorldCircleConstraint(stripeE, false);
	rmBuildArea(stripeE);
