...
void main(void)
{
	...
	string hinterForest   = "Italian Forest";    // 16 shipped maps use this
	                                             // name, and so does the mod's
	                                             // own anatolia branch. "z42
	                                             // Italian Forest" appears in 3
	                                             // real maps and Black Sea's
	                                             // census shows no Italian trees
	                                             // from it - likely a dead name.
	...
	// that: the surplus is measured at generation time and taken back with a
	// cliff, which is ground you cannot build on.
	float balanceRise     = 6.0;  // how high the cliff stands over the country
	int   balanceKeep     = 6;    // tiles of the surplus left flat, so the
	                              // south stays a shade roomier, not smaller
	...
	// ===== ISOLATION TEST - DELETE ONCE THE FOREST QUESTION IS SETTLED =====
	// A plain island in open water (x 0.15, z 0.50 - the census says nothing
	// else is out there) carrying the SAME forest type and settings, with NO
	// constraints at all. If trees appear here the forest call is sound and
	// the hinterland is at fault; if they do not, the forest call is.
	shoreScaffold(0.15, 0.50, 900);
	int testIsleBox = rmCreateBoxConstraint("test isle box",
		0.09, 0.44, 0.21, 0.56, 0.01);
	int failCount = 0;
	// forest test - Black Sea's forest idiom (000_blacksea.xs 1146-1173): no explicit
	// location, the constraints find the ground; the build result is CHECKED so
	// a failure is visible instead of silent.
	failCount = 0;
	for (ft = 0; < 40)
	{
		int forestTest = rmCreateArea("forest test " + ft);
		rmSetAreaWarnFailure(forestTest, false);
		rmSetAreaSize(forestTest, rmAreaTilesToFraction(100), rmAreaTilesToFraction(200));
		rmSetAreaForestType(forestTest, hinterForest);
		rmSetAreaForestDensity(forestTest, 1.0);
		rmAddAreaToClass(forestTest, rmClassID("classForest"));
		rmSetAreaForestClumpiness(forestTest, 0.0);
		rmSetAreaForestUnderbrush(forestTest, 0.0);
		rmSetAreaCoherence(forestTest, 0.4);
		rmAddAreaConstraint(forestTest, testIsleBox);
		rmAddAreaConstraint(forestTest, avoidStreet);
		if (rmBuildArea(forestTest) == false)
		{
			failCount++;
	...
	// north wood - Black Sea's forest idiom (000_blacksea.xs 1146-1173): no explicit
	// location, the constraints find the ground; the build result is CHECKED so
	// a failure is visible instead of silent.
	failCount = 0;
	for (fn = 0; < 40)
	{
		int forestN = rmCreateArea("north wood " + fn);
		rmSetAreaWarnFailure(forestN, false);
		rmSetAreaSize(forestN, rmAreaTilesToFraction(100), rmAreaTilesToFraction(200));
		rmSetAreaForestType(forestN, hinterForest);
		rmSetAreaForestDensity(forestN, 1.0);
		rmAddAreaToClass(forestN, rmClassID("classForest"));
		rmSetAreaForestClumpiness(forestN, 0.0);
		rmSetAreaForestUnderbrush(forestN, 0.0);
		rmSetAreaCoherence(forestN, 0.4);
		rmAddAreaConstraint(forestN, hinterNbox);
		rmAddAreaConstraint(forestN, avoidStreet);
		if (rmBuildArea(forestN) == false)
		{
			failCount++;
			if (failCount == 5) { break; }
		}
	...
	// south wood - Black Sea's forest idiom (000_blacksea.xs 1146-1173): no explicit
	// location, the constraints find the ground; the build result is CHECKED so
	// a failure is visible instead of silent.
	failCount = 0;
	for (fs = 0; < 40)
	{
		int forestS = rmCreateArea("south wood " + fs);
		rmSetAreaWarnFailure(forestS, false);
		rmSetAreaSize(forestS, rmAreaTilesToFraction(100), rmAreaTilesToFraction(200));
		rmSetAreaForestType(forestS, hinterForest);
		rmSetAreaForestDensity(forestS, 1.0);
		rmAddAreaToClass(forestS, rmClassID("classForest"));
		rmSetAreaForestClumpiness(forestS, 0.0);
		rmSetAreaForestUnderbrush(forestS, 0.0);
		rmSetAreaCoherence(forestS, 0.4);
		rmAddAreaConstraint(forestS, hinterSbox);
		rmAddAreaConstraint(forestS, avoidStreet);
		if (rmBuildArea(forestS) == false)
		{
			failCount++;
			if (failCount == 5) { break; }
		}
	...
	float surplus = southRoom - northRoom - rmZTilesToFraction(balanceKeep);
	rmEchoInfo("countryside room  north " + northRoom + "  south " + southRoom
		+ "  -> cliff takes " + surplus);
	if (surplus > 0.0)
	{
		// Sits at the far end, against the south map edge, so it eats the
		// extra depth and never pinches the neck joining city to country.
		float cliffFar  = hinterEdgeS - southRoom;
		int balanceCliffbox = rmCreateBoxConstraint("south balance box",
			sx1 - flankProm - hinterSide, cliffFar,
			sx5 + margin + hinterSide, cliffFar + surplus, 0.01);
		int balanceCliff = rmCreateArea("south balance cliff");
		rmSetAreaWarnFailure(balanceCliff, false);
		rmSetAreaSize(balanceCliff, 0.4, 0.4);
		rmSetAreaLocation(balanceCliff, sx3, cliffFar + surplus * 0.5);
		...
		rmAddAreaConstraint(balanceCliff, balanceCliffbox);
		rmAddAreaConstraint(balanceCliff, hinterSbox);
		rmAddAreaConstraint(balanceCliff, avoidStreet);
