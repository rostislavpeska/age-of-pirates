...
// --- the cliffs: coherent blobs shaped ONLY by the constraints ---------
	...
	// ---- WALL-DOCK CLIFFS - the Paris idiom (0000_paris_dansil.xs 370,
	// 1110-1137). The type constraint at 0.001 m lets each cliff flood right
	// up to the wall units and STOP - docking against the wall without
	// burying it. One cliff beyond each wall end seals the walk-around gap.
	// They deliberately do NOT avoid the street: the wall stands on city
	// ground now, so the docks must sit against it; city BLOCKS are protected
	// by their own class constraint instead (Paris's avoidBlock).
	...
	int dockAvoidBlocks = rmCreateClassDistanceConstraint("docks off city blocks",
		rmClassID("classBlock"), 4.0);
	int   wallDockTiles = 47;  // seed distance past each wall end (half-wall 40.5)

	// north wall, back end
	int dock1 = rmCreateArea("wall dock 1");
	rmSetAreaWarnFailure(dock1, false);
	rmSetAreaSize(dock1, 0.03, 0.03);
	...
	rmSetAreaCliffType(dock1, hinterCliff);
	...
	rmAddAreaConstraint(dock3, avoidGuardW);
	rmAddAreaConstraint(dock3, avoidGuardE);
	...
	rmSetAreaCliffType(dock4, hinterCliff);
