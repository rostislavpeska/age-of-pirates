...
void main(void)
{
	...
	// ---- FLANKING COUNTRYSIDE: high cliff land either side of the cities --
	// Two masses in the open water flanks. Hardcoded, same recipe as the
	// balance cliff. They keep clear of the lanes but deliberately do NOT
	// wall the shore off from them - cornerCliffLane is what leaves that gap.
	float cornerCliffWX   = 0.13;  // west mass, beside the north island
	float cornerCliffWZ   = 0.70;
	float cornerCliffEX   = 0.87;  // east mass, beside the south island
	float cornerCliffEZ   = 0.30;
	float cornerCliffSize = 0.030; // area as a fraction of the map
	float cornerCliffLen  = 0.10;  // half-length, stretches it into a ridge
	float cornerCliffRise = 14.0;  // VERY high - much taller than the balance cliff
	float cornerCliffLane = 60.0;  // metres kept clear of the trade routes
	int   cornerCliffLand = 2600; // tiles of land laid down under each mass
	...
	// How wide the lane corner is rounded, in tiles. 0 would give the old
	// right angle; bigger = a longer, lazier sweep.
	int   laneCurve       = 14;
	...
	float nCurveX = rmXTilesToFraction(laneCurve);
	float nCurveZ = rmZTilesToFraction(laneCurve);
	float sCurveX = rmXTilesToFraction(laneCurve);
	float sCurveZ = rmZTilesToFraction(laneCurve);
	...
	// corner rounded: approach, diagonal cut, exit - instead of one hard turn
	rmAddTradeRouteWaypoint(tradeRouteN, nBendX, nRouteAsk + nCurveZ);
	rmAddTradeRouteWaypoint(tradeRouteN, nBendX - nCurveX * 0.45,
		nRouteAsk + nCurveZ * 0.45);
	rmAddTradeRouteWaypoint(tradeRouteN, nBendX - nCurveX, nRouteAsk);
	...
	rmAddTradeRouteWaypoint(tradeRouteS, sBendX, sRouteAsk - sCurveZ);
	rmAddTradeRouteWaypoint(tradeRouteS, sBendX + sCurveX * 0.45,
		sRouteAsk - sCurveZ * 0.45);
	rmAddTradeRouteWaypoint(tradeRouteS, sBendX + sCurveX, sRouteAsk);
	...
	rmSetAreaSize(hinterN, rmAreaTilesToFraction(600),
		rmAreaTilesToFraction(2600));
	...
	rmSetAreaSize(hinterS, rmAreaTilesToFraction(600),
		rmAreaTilesToFraction(2600));
	...
	rmSetAreaSize(grassTopN, rmAreaTilesToFraction(600),
		rmAreaTilesToFraction(2600));
	...
	rmSetAreaSize(grassTopS, rmAreaTilesToFraction(600),
		rmAreaTilesToFraction(2600));
	...
	int balanceCliff = rmCreateArea("south balance cliff");
	rmSetAreaWarnFailure(balanceCliff, false);
	rmSetAreaSize(balanceCliff, balanceCliffSize, balanceCliffSize);
	rmSetAreaLocation(balanceCliff, balanceCliffX, balanceCliffZ);
	// stretched along x, so it reads as a ridge rather than a dome
	rmAddAreaInfluenceSegment(balanceCliff,
		balanceCliffX - balanceCliffLen, balanceCliffZ,
		balanceCliffX + balanceCliffLen, balanceCliffZ);
	rmSetAreaCoherence(balanceCliff, 1.0);
	rmSetAreaBaseHeight(balanceCliff, cityHeight + balanceRise);
	rmSetAreaCliffType(balanceCliff, hinterCliff);
	rmSetAreaCliffEdge(balanceCliff, 1, 1.0, 0.1, 1.0, 0);
	rmSetAreaCliffHeight(balanceCliff, 0, 0.0, 1.0);
	rmAddAreaToClass(balanceCliff, rmClassID("classPlateau"));
	rmAddAreaToClass(balanceCliff, rmClassID("classCliff"));
	rmSetAreaObeyWorldCircleConstraint(balanceCliff, false);
	rmBuildArea(balanceCliff);

	// --- flanking countryside: high cliff land in the open flanks ---------
	// Distance is METRES here, like every other constraint in the engine.
	int avoidLanes = rmCreateTradeRouteDistanceConstraint("cliffs off the lanes",
		cornerCliffLane);

	// LAND FIRST. This map is water-initialised: an area with a base height
	// over open water has nothing to cut cliff edges into, which is why the
	// balance cliff works (it sits on the countryside) and these did not.
	// shoreScaffold is the same helper the gun islands use.
	shoreScaffold(cornerCliffWX, cornerCliffWZ, cornerCliffLand);
	shoreScaffold(cornerCliffEX, cornerCliffEZ, cornerCliffLand);

	int cornerCliffW = rmCreateArea("west flank cliff");
	rmSetAreaWarnFailure(cornerCliffW, true);
	rmSetAreaSize(cornerCliffW, cornerCliffSize, cornerCliffSize);
	rmSetAreaLocation(cornerCliffW, cornerCliffWX, cornerCliffWZ);
	rmAddAreaInfluenceSegment(cornerCliffW, cornerCliffWX - cornerCliffLen, cornerCliffWZ,
		cornerCliffWX + cornerCliffLen, cornerCliffWZ);
	rmSetAreaCoherence(cornerCliffW, 1.0);
	rmSetAreaBaseHeight(cornerCliffW, cityHeight + cornerCliffRise);
	rmSetAreaCliffType(cornerCliffW, hinterCliff);
	rmSetAreaCliffEdge(cornerCliffW, 1, 1.0, 0.1, 1.0, 0);
	rmSetAreaCliffHeight(cornerCliffW, 0, 0.0, 1.0);
	rmSetAreaMix(cornerCliffW, hinterMix);
	rmAddAreaConstraint(cornerCliffW, avoidLanes);
	rmAddAreaToClass(cornerCliffW, rmClassID("classPlateau"));
	rmAddAreaToClass(cornerCliffW, rmClassID("classCliff"));
	rmSetAreaObeyWorldCircleConstraint(cornerCliffW, false);
	rmBuildArea(cornerCliffW);

	/*int cornerCliffE = rmCreateArea("east flank cliff");
	rmSetAreaWarnFailure(cornerCliffE, true);
	rmSetAreaSize(cornerCliffE, cornerCliffSize, cornerCliffSize);
	rmSetAreaLocation(cornerCliffE, cornerCliffEX, cornerCliffEZ);
	rmAddAreaInfluenceSegment(cornerCliffE, cornerCliffEX - cornerCliffLen, cornerCliffEZ,
		cornerCliffEX + cornerCliffLen, cornerCliffEZ);
	rmSetAreaCoherence(cornerCliffE, 1.0);
	rmSetAreaBaseHeight(cornerCliffE, cityHeight + cornerCliffRise);
	rmSetAreaCliffType(cornerCliffE, hinterCliff);
	rmSetAreaCliffEdge(cornerCliffE, 1, 1.0, 0.1, 1.0, 0);
	rmSetAreaCliffHeight(cornerCliffE, 0, 0.0, 1.0);
	rmSetAreaMix(cornerCliffE, hinterMix);
	rmAddAreaConstraint(cornerCliffE, avoidLanes);
	rmAddAreaToClass(cornerCliffE, rmClassID("classPlateau"));
	rmAddAreaToClass(cornerCliffE, rmClassID("classCliff"));
	rmSetAreaObeyWorldCircleConstraint(cornerCliffE, false);
	rmBuildArea(cornerCliffE);*/
