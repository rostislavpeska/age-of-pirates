...
void main(void)
{
	...
	// --- WILD FLANK COUNTRYSIDE - zp_zaustralia.xs big-island idiom -------
	// Every number below is DERIVED, not guessed (guide 137-145: the minimap
	// is rotated 45 deg - visual left = code low-x/high-z, visual right =
	// code high-x/low-z):
	//   north island west edge x=0.3527 (z 0.586..0.944) -> spine at 0.34
	//   south island east edge x=0.6140 (z 0.056..0.414) -> spine at 0.63
	//   lane clearance: every point >= 0.037 from every lane segment
	//   rim: every point inside r=0.48 of the world circle
	// The boxes are code-axis fences (guide: box constraints are axis-
	// aligned): the masses cannot leak into the channel or over an island.
	int wildAvoidLane = rmCreateTradeRouteDistanceConstraint("wild flank off lanes", 10.0);
	// z floor 0.56, not 0.50: the north lane snaps up to ~4 tiles, so the box
	// stays clear of where the channel run can actually land
	int wildWbox = rmCreateBoxConstraint("west wild box", 0.02, 0.56, 0.3527, 0.98, 0.01);
	int wildEbox = rmCreateBoxConstraint("east wild box", 0.6140, 0.02, 0.98, 0.44, 0.01);

	// WEST land: hugs the north island west shore, sweeps to the rim
	int wildW = rmCreateArea("west wild land");
	rmSetAreaWarnFailure(wildW, false);
	rmSetAreaSize(wildW, 0.12, 0.12);
	rmSetAreaCoherence(wildW, 0.95);
	rmSetAreaBaseHeight(wildW, 2.0);
	rmSetAreaSmoothDistance(wildW, 20);
	rmSetAreaMix(wildW, hinterMix);
	rmSetAreaElevationType(wildW, cElevTurbulence);
	rmSetAreaElevationVariation(wildW, 4.0);
	rmSetAreaElevationMinFrequency(wildW, 0.09);
	rmSetAreaElevationOctaves(wildW, 3);
	rmSetAreaElevationPersistence(wildW, 0.2);
	rmSetAreaElevationNoiseBias(wildW, 1);
	rmAddAreaToClass(wildW, rmClassID("classPlateau"));
	rmAddAreaConstraint(wildW, wildAvoidLane);
	rmAddAreaConstraint(wildW, wildWbox);
	rmSetAreaObeyWorldCircleConstraint(wildW, false);
	rmAddAreaInfluenceSegment(wildW, 0.34, 0.62, 0.34, 0.90);
	rmAddAreaInfluenceSegment(wildW, 0.26, 0.78, 0.16, 0.74);
	rmAddAreaInfluenceSegment(wildW, 0.12, 0.68, 0.10, 0.62);
	rmAddAreaInfluenceSegment(wildW, 0.31, 0.59, 0.33, 0.60);
	rmBuildArea(wildW);

	// high cliff along its spine
	int wildWtop = rmCreateArea("west wild cliff");
	rmSetAreaWarnFailure(wildWtop, false);
	rmSetAreaSize(wildWtop, 0.04, 0.04);
	rmSetAreaCoherence(wildWtop, 0.95);
	rmSetAreaBaseHeight(wildWtop, cityHeight + 14.0);
	rmSetAreaSmoothDistance(wildWtop, 20);
	rmSetAreaMix(wildWtop, hinterMix);
	rmSetAreaCliffType(wildWtop, hinterCliff);
	rmSetAreaCliffEdge(wildWtop, 1, 1.0, 0.1, 1.0, 0);
	rmSetAreaCliffHeight(wildWtop, 0, 0.0, 1.0);
	rmAddAreaToClass(wildWtop, rmClassID("classCliff"));
	rmAddAreaToClass(wildWtop, rmClassID("classPlateau"));
	rmAddAreaConstraint(wildWtop, wildAvoidLane);
	rmAddAreaConstraint(wildWtop, wildWbox);
	rmSetAreaObeyWorldCircleConstraint(wildWtop, false);
	rmAddAreaInfluenceSegment(wildWtop, 0.32, 0.66, 0.32, 0.84);
	rmAddAreaInfluenceSegment(wildWtop, 0.24, 0.76, 0.18, 0.73);
	rmBuildArea(wildWtop);

	// EAST land: hugs the south island east shore, sweeps to the rim
	int wildE = rmCreateArea("east wild land");
	rmSetAreaWarnFailure(wildE, false);
	rmSetAreaSize(wildE, 0.12, 0.12);
	rmSetAreaCoherence(wildE, 0.95);
	rmSetAreaBaseHeight(wildE, 2.0);
	rmSetAreaSmoothDistance(wildE, 20);
	rmSetAreaMix(wildE, hinterMix);
	rmSetAreaElevationType(wildE, cElevTurbulence);
	rmSetAreaElevationVariation(wildE, 4.0);
	rmSetAreaElevationMinFrequency(wildE, 0.09);
	rmSetAreaElevationOctaves(wildE, 3);
	rmSetAreaElevationPersistence(wildE, 0.2);
	rmSetAreaElevationNoiseBias(wildE, 1);
	rmAddAreaToClass(wildE, rmClassID("classPlateau"));
	rmAddAreaConstraint(wildE, wildAvoidLane);
	rmAddAreaConstraint(wildE, wildEbox);
	rmSetAreaObeyWorldCircleConstraint(wildE, false);
	rmAddAreaInfluenceSegment(wildE, 0.63, 0.10, 0.63, 0.36);
	rmAddAreaInfluenceSegment(wildE, 0.74, 0.22, 0.84, 0.26);
	rmAddAreaInfluenceSegment(wildE, 0.88, 0.32, 0.90, 0.38);
	rmAddAreaInfluenceSegment(wildE, 0.69, 0.41, 0.67, 0.40);
	rmBuildArea(wildE);

	// high cliff along its spine
	int wildEtop = rmCreateArea("east wild cliff");
	rmSetAreaWarnFailure(wildEtop, false);
	rmSetAreaSize(wildEtop, 0.04, 0.04);
	rmSetAreaCoherence(wildEtop, 0.95);
	rmSetAreaBaseHeight(wildEtop, cityHeight + 14.0);
	rmSetAreaSmoothDistance(wildEtop, 20);
	rmSetAreaMix(wildEtop, hinterMix);
	rmSetAreaCliffType(wildEtop, hinterCliff);
	rmSetAreaCliffEdge(wildEtop, 1, 1.0, 0.1, 1.0, 0);
	rmSetAreaCliffHeight(wildEtop, 0, 0.0, 1.0);
	rmAddAreaToClass(wildEtop, rmClassID("classCliff"));
	rmAddAreaToClass(wildEtop, rmClassID("classPlateau"));
	rmAddAreaConstraint(wildEtop, wildAvoidLane);
	rmAddAreaConstraint(wildEtop, wildEbox);
	rmSetAreaObeyWorldCircleConstraint(wildEtop, false);
	rmAddAreaInfluenceSegment(wildEtop, 0.65, 0.14, 0.65, 0.32);
	rmAddAreaInfluenceSegment(wildEtop, 0.76, 0.24, 0.82, 0.27);
	rmBuildArea(wildEtop);
