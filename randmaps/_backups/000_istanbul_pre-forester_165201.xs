...
void main(void)
{
	...
	int avoidCliff = rmCreateClassDistanceConstraint("trees off the cliff",
		rmClassID("classCliff"), rmZTilesToFraction(1));
	...
	// ROWS 5 AND 6 ARE THEIR OWN ZONE (*_BACK). Resource buildings are placed
	// by placeGroupings() into *_SUBURB, which is row 4 ONLY, so nothing
	// economic can reach the back rows - they are reserved for the player.
	// fillHouses() runs over CENTRE..BACK, so leftover back cells still get
	// houses. Each cell stores col*100 + row for the no-repeat house weave.
	const int N_CENTRE_START =  0;   const int N_CENTRE_END =  7;
	const int N_SUBURB_START =  8;   const int N_SUBURB_END = 11;   // row 4 only
	const int N_BACK_START   = 12;   const int N_BACK_END   = 19;   // rows 5 + 6
	const int S_CENTRE_START = 20;   const int S_CENTRE_END = 27;
	const int S_SUBURB_START = 28;   const int S_SUBURB_END = 31;   // row 4 only
	const int S_BACK_START   = 32;   const int S_BACK_END   = 39;   // rows 5 + 6
	...
	// north suburb: ROW 4 ONLY, minus the factory corner - resource buildings live here
	...
	// north back: row 5 minus both construction corners, then all of row 6 - houses only
	...
	xsArraySetVector(gCityLocs, 17, xsVectorSet(nx3, 0.0, nz6));   xsArraySetInt(gCityCode, 17, 306);
	...
	xsArraySetVector(gCityLocs, 27, xsVectorSet(sx5, 0.0, sz3));   xsArraySetInt(gCityCode, 27, 503);
	...
	// south back: row 5 minus corners, then all of row 6 - houses only
	...
	xsArraySetVector(gCityLocs, 35, xsVectorSet(sx1, 0.0, sz6));   xsArraySetInt(gCityCode, 35, 106);
	xsArraySetVector(gCityLocs, 36, xsVectorSet(sx2, 0.0, sz6));   xsArraySetInt(gCityCode, 36, 206);
	xsArraySetVector(gCityLocs, 37, xsVectorSet(sx3, 0.0, sz6));   xsArraySetInt(gCityCode, 37, 306);
