"""aoe-xml verification: the one command after any XML change in the mod. Run from the repo root.
    ...
    python .claude/skills/aoe-xml/scripts/xmlcheck.py                 # whole mod
    python .claude/skills/aoe-xml/scripts/xmlcheck.py PATH [PATH...]  # only these files / folders (+ protos they define)
    python .claude/skills/aoe-xml/scripts/xmlcheck.py --no-archive    # skip the .bar index (offline / fast)
...
os.chdir(REPO)
sys.path.insert(0, os.path.join(REPO, '.claude', 'skills', 'bar-extract', 'scripts'))
BS = chr(92)
