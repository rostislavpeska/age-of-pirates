...
DUMPS = sorted((PROFILE / "RandMaps").glob("Age3DERM*.dmp.txt"), key=lambda p: p.stat().st_mtime)
pytestmark = pytest.mark.skipif(not DUMPS, reason="no CXSDump in the profile folder (the game writes one per generation)")


def _builtins():
    return xsc.builtins_from_dump(DUMPS[-1])
