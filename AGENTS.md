...
## Any XML -> read `skills/aoe-xml/SKILL.md` first, verify with `python skills/aoe-xml/scripts/xmlcheck.py`
...
4. **`data/*.xml` edits are inert until the `.xml.xmb` twin is rebuilt**
   (`python skills/aoe3de-bar-archives/scripts/xmbc.py check|build data/<file>.xml`); commit both.
...
- Skills: `skills/` - `bar-extract` (vanilla files, XML<->XMB), `aoe-building-pipeline` (buildings,
...
All skills are authored in `skills/`, including the original AoP skills, Blender authoring,
reusable AoE3DE tooling and public export/import. Read and edit that directory directly.
Choose relevant packages from `skills/*/SKILL.md` by their names and frontmatter
...
Native skill discovery uses two ignored links to the same directory:
`.agents/skills` for Codex, Cursor, Gemini CLI and VS Code Copilot; `.claude/skills`
for Claude Code. These are aliases, never independently edited or synchronized copies.
After a fresh clone, run `python scripts/tools/setup_agent_skill_link.py`;
...
Older commits tracked `.claude/skills`; inspect those versions in a separate checkout.
Never restore historical files through the current discovery junction.
...
For geometry use `skills/blender-architecture/SKILL.md`; for UVs and textures use
`skills/blender-architecture-texturing/SKILL.md`. Building work in this mod starts with
`skills/aoe-building-pipeline/SKILL.md`, which adds the AoP profile to the reusable workflow.
Archive/XMB work starts with `skills/bar-extract/SKILL.md`, then its referenced
...
Public repositories receive only allowlisted snapshots. Use `skills/public-skill-export/SKILL.md`
or `skills/public-skill-import/SKILL.md`. Device paths live in ignored
