...
void main(void)
{
  ...
  int nativePattern = rmRandInt(0,1);
   ...
   rmAddAreaInfluenceSegment(bigIslandID, .5, .56, .37, .5);
	...
	rmAddObjectDefItem(pearlNWID, "zpPearlSource", 1, 0);
	...
	rmAddObjectDefItem(pearlSEID, "zpPearlSource", 1, 0);
   ...
   rmAddTradeRouteWaypoint(tradeRouteID, 0.82, 0.82);  // Swapped from (0.82, 0.82)
   rmAddTradeRouteWaypoint(tradeRouteID, 0.5, 0.95);   // Swapped from (0.95, 0.5)
   rmAddTradeRouteWaypoint(tradeRouteID, 0.18, 0.82);  // Swapped from (0.82, 0.18)
   rmAddTradeRouteWaypoint(tradeRouteID, 0.05, 0.5);   // Swapped from (0.5, 0.05)
   rmAddTradeRouteWaypoint(tradeRouteID, 0.18, 0.18);  // Swapped from (0.18, 0.18)
   rmAddTradeRouteWaypoint(tradeRouteID, 0.5, 0.05);   // Swapped from (0.05, 0.5)
   rmAddTradeRouteWaypoint(tradeRouteID, 0.82, 0.18);  // Swapped from (0.18, 0.82)
   rmAddTradeRouteWaypoint(tradeRouteID, 0.95, 0.5);   // Swapped from (0.5, 0.95)
   rmAddTradeRouteWaypoint(tradeRouteID, 0.82, 0.82);  // Swapped from (0.82, 0.82)
   ...
   for (i=0; <numPirateSettlements) {
      ...
      rmSetObjectDefMaxDistance(pirateControllerID, 30.0);
   ...
   rmPlaceObjectDefInArea(KotHID, 0, bigIslandID, 1);
      ...
      for(i=0; <socketCount) {
         ...
         if (cNumberNonGaiaPlayers == 4) {
            ...
            float harbourOffsetDistance = 18.0; // Default offset
            ...
            if ((dirToSocketX > 0) && (dirToSocketZ > 0)) {
               ...
               harbourOffsetDistance = 13.0; // Special offset for East-facing harbour
               ...
               harbourOffsetDistance = 20.0; // Special offset for East-facing harbour
	...
	for(i=1; <cNumberPlayers) {
      ...
      rmPlaceObjectDefAtLoc(waterSpawnPointID, i, rmXMetersToFraction(xsVectorGetX(closestPoint)), rmZMetersToFraction(xsVectorGetZ(closestPoint)));
		...
		if (getsHabsburg == 1) {
			...
			playerNativeID = rmCreateGrouping("player "+i+" native", "Jesuit_Cathedral_EU_0"+jesuitVar);
