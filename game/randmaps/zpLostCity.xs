...
// updated December 2021 by vividlyplain
...
void main(void)
{
   ...
   rmSetSeaType("Yucatan Coast Alt");
...
//   int longPlayerConstraint=rmCreateClassDistanceConstraint("land stays away from players", classPlayer, 24.0);
  ...
  // int nearWater10 = rmCreateTerrainDistanceConstraint("near water", "Water", true, 10.0);
   ...
   int whaleVsWhaleID=rmCreateTypeDistanceConstraint("whale v whale", "AbstractWhale", 24.0);
   ...
   int avoidFood=rmCreateTypeDistanceConstraint("food avoids food", "huntable", 64.0);
   ...
   int avoidCoin=rmCreateTypeDistanceConstraint("avoid coin", "gold", 88.0);
   ...
   int smallavoidCliff=rmCreateClassDistanceConstraint("small vs. cliff", rmClassID("classCliff"), 4.0);
   ...
   int avoidNugget=rmCreateTypeDistanceConstraint("nugget vs nugget", "abstractnugget", 84.0);
   ...
   int avoidNuggets=rmCreateClassDistanceConstraint("stuff avoids nuggets", rmClassID("classNuggets"), 64.0);
...
//********************************************  Place Trade Route  *******************************************
   ...
   rmSetObjectDefMaxDistance(socketID, 8.0);
	...
	if (tradeRouteLoc < 0.33) // Center
	{
		...
		rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.5, 0.5, 8, 10);
		rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.5, 0.05, 8, 10);
	}
...
//***************************************  Starting Resources  ********************************************  
   ...
   //rmPlaceObjectDefAtLoc(playerSilver2ID, 0, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
    ...
    rmAddObjectDefItem(baseHunt1, "capybara", 8, 3.0);
    rmSetObjectDefMinDistance(baseHunt1, 12.0);
    rmSetObjectDefMaxDistance(baseHunt1, 14.0);
    rmAddObjectDefConstraint(baseHunt1, avoidAll);
...
*/
  ...
  // ************************  Define and place Cliffs then nuggets  *******************************
   numTries=1.5*cNumberNonGaiaPlayers+1;
   failCount=0;
   for(i=0; <numTries)
   {
	  ...
	  rmSetAreaCliffType(cliffID, "Yucatan");
  ...
  // Another cliff
   ...
   for(i=0; <numTries)
   {
      ...
      rmSetAreaCliffType(cliff2ID, "Yucatan");
...
// *********************  Place fish in both water bodies  ****************************
   ...
   rmAddObjectDefItem(whale2ID, "MinkeWhale", 1, 9.0);
...
//  ***************************  Define and place Forests  *******************************
   int forestTreeID = 0;
   numTries=4+6*cNumberNonGaiaPlayers;
   failCount=0;
   for (i=0; <numTries)
   {
      ...
      {   
	   ...
	   {
       numTries=10+10*cNumberNonGaiaPlayers;
	   int forest2=rmCreateArea("forest2"+i);
	   rmSetAreaWarnFailure(forest2, false);
	   rmSetAreaObeyWorldCircleConstraint(forest2, true);
	   rmSetAreaSize(forest2, rmAreaTilesToFraction(100), rmAreaTilesToFraction(250));
	   ...
	   rmAddAreaConstraint(forest2, avoidCW); 
	   rmAddAreaConstraint(forest2, avoidTC); 
...
// **************************  Resources that can be placed after forests (Hunting)  ***************************
	...
	int tapirCount = rmRandInt(6,6);
	int capyCount = rmRandInt(8,8);

   int tapirID=rmCreateObjectDef("NW tapir crash");
   rmAddObjectDefItem(tapirID, "tapir", tapirCount, 3.0);
   ...
   rmPlaceObjectDefAtLoc(tapirID, 0, 0.5, 0.5, tapirCount*0.75*cNumberNonGaiaPlayers);

	int tapir2ID=rmCreateObjectDef("SE tapir crash");
   rmAddObjectDefItem(tapir2ID, "tapir", tapirCount, 3.0);
   ...
   rmPlaceObjectDefAtLoc(tapir2ID, 0, 0.5, 0.5, tapirCount*0.75*cNumberNonGaiaPlayers);

	int capybaraID=rmCreateObjectDef("NW capybara pack");
   rmAddObjectDefItem(capybaraID, "capybara", capyCount, 3.0);
   ...
   rmSetObjectDefCreateHerd(capybaraID, true);
	rmPlaceObjectDefAtLoc(capybaraID, 0, 0.5, 0.5, capyCount*2.25*cNumberNonGaiaPlayers);

	int capybara2ID=rmCreateObjectDef("SE capybara pack");
   rmAddObjectDefItem(capybara2ID, "capybara", capyCount, 3.0);
   ...
   rmSetObjectDefCreateHerd(capybara2ID, true);
	rmPlaceObjectDefAtLoc(capybara2ID, 0, 0.5, 0.5, capyCount*2.25*cNumberNonGaiaPlayers);
  ...
  */
  // Text
   ...
   rmSetObjectDefMaxDistance(nugget2, 80.0);
   ...
   int nugget22= rmCreateObjectDef("nugget tier2"); 
   rmAddObjectDefItem(nugget22, "Nugget", 1, 0.0);
   rmSetNuggetDifficulty(2, 2);
   rmSetObjectDefMinDistance(nugget22, 0.0);
   rmSetObjectDefMaxDistance(nugget22, rmXFractionToMeters(0.5));
   rmAddObjectDefConstraint(nugget22, shortAvoidImpassableLand);
   rmAddObjectDefConstraint(nugget22, avoidNugget);
   rmAddObjectDefConstraint(nugget22, avoidTC);
   rmAddObjectDefConstraint(nugget22, avoidCW);
   rmAddObjectDefConstraint(nugget22, avoidTradeRoute);
   rmAddObjectDefConstraint(nugget22, avoidAll);
   rmAddObjectDefConstraint(nugget22, avoidWater12);
   rmAddObjectDefConstraint(nugget22, nuggetAvoidEdgeConstraint);
   rmAddObjectDefToClass(nugget22, rmClassID("classNuggets"));
   rmPlaceObjectDefAtLoc(nugget22, 0, 0.5, 0.5, 2*cNumberNonGaiaPlayers);
