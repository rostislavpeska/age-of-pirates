...
void main(void)
{
  ...
  int avoidTCLong=rmCreateTypeDistanceConstraint("stay away from TC by far", "TownCenter", 30.0);
	...
	// Island Constraints  
	int islandConstraint=rmCreateClassDistanceConstraint("islands avoid each other", classIsland, 20.0);
  ...
  int avoidExtraIslands=rmCreateClassDistanceConstraint("stuff avoids extra islands", classExtraIsland, 20.0);
	...
	rmSetAreaSize(bigIslandID, rmAreaTilesToFraction(700.0), rmAreaTilesToFraction(700.0));
  ...
  // Native islands  
	...
	rmSetAreaSize(nativeIsland1ID, rmAreaTilesToFraction(700.0), rmAreaTilesToFraction(700.0));
  ...
  int nativeIsland2ID=rmCreateArea("native island 2");
	rmSetAreaSize(nativeIsland2ID, rmAreaTilesToFraction(700.0), rmAreaTilesToFraction(700.0));
  ...
  int nativeIsland3ID=rmCreateArea("native island 3");
	rmSetAreaSize(nativeIsland3ID, rmAreaTilesToFraction(700.0), rmAreaTilesToFraction(700.0));
  ...
  int nativeIsland4ID=rmCreateArea("native island 4");
	rmSetAreaSize(nativeIsland4ID, rmAreaTilesToFraction(700.0), rmAreaTilesToFraction(700.0));
  ...
  int northIsland=rmCreateArea("north "+i);
    rmSetAreaSize(northIsland, rmAreaTilesToFraction(800.0), rmAreaTilesToFraction(800.0));
    ...
    rmSetAreaSize(southIsland, rmAreaTilesToFraction(800.0), rmAreaTilesToFraction(800.0));
    ...
    rmSetAreaSize(eastIsland, rmAreaTilesToFraction(500.0), rmAreaTilesToFraction(500.0));
    ...
    rmSetAreaSize(westIsland, rmAreaTilesToFraction(500.0), rmAreaTilesToFraction(500.0));
  ...
  for (i=0; <numTries) {   
    ...
    rmSetAreaSize(extraIsland, rmAreaTilesToFraction(400), rmAreaTilesToFraction(400));
    ...
    /*rmSetAreaForestType(extraIsland, forestType);
    rmSetAreaForestDensity(extraIsland, 0.6);
    rmSetAreaForestClumpiness(extraIsland, 0.1);
    rmSetAreaForestUnderbrush(extraIsland, 0.6);*/
...
// Gold
  ...
  for (i=0; <numTries) {   
    ...
    rmAddAreaConstraint(forest, avoidStartingIslands);
    rmAddAreaConstraint(forest, avoidNativeIslands);
    rmAddAreaConstraint(forest, avoidBuildingIslands);
    rmAddAreaConstraint(forest, avoidTradeIslands);
...
rmPlaceObjectDefAtLoc(nugget1, 0, 0.5, 0.5, cNumberNonGaiaPlayers*5); 
  ...
  //Place random whales everywhere --------------------------------------------------------
	...
	rmPlaceObjectDefAtLoc(whaleID, 0, 0.5, 0.5, cNumberNonGaiaPlayers*7); 
...
rmAddObjectDefConstraint(nugget2, ObjectAvoidTradeRoute);
