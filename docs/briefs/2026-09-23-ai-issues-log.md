...
| I14 | 2026-09-24, twin test (2880x1800) | a second Play click armed the in-match FLARE mode ('Left-click in the world or Mini Map to send flare'): the next minimap click would send a flare | on this device the match loads fast; the lobby's Play pixel (2471,1708) is the in-match flare button, so a re-click after the load lands in the match (the driver re-clicks Play when the lobby 'is still up 6 s later') | open, for the AI campaign: before the second click, re-probe `lobby_probe` on a fresh capture and click only when it still matches; Esc cancels an armed flare |
| I15 | runs 31-34, run 35 | London AIs made dock plans that vanished; 0 docks in every run | the engine's dock-site search found no buildable shore: London had quay cliffs and no beaches (every failure 'placement failed with state (3)') | **map fix, closed** (owner 2026-09-24, '5 docks is enough'): the city ramps and slipways (9c11b233, f2b234a5) gave run 35 five docks (P2 3 from 4:01, P4 1, P5 1); P3 still 0 after 21 failures - accepted, the AI does not land troops on London anyway |
| I16 | 2026-09-25, run 14 (owner watching) | pirate ships parked at a Naval Fort; almost no ships; an Extreme AI playing like Easy | `570d65a2` (2026-08-27) deleted the Istanbul fleet split (gun fleet, gun raid, monitor maintain) without the owner's approval and pruned the criteria that measured it; the suite stayed green; v7.2 shipped without it | **restored** (brief `2026-09-25-istanbul-fleet-split-restore.md`), `OWNER_FEATURES` test; further guards proposed in `2026-09-25-ai-regression-prevention.md` |
| I17 | 2026-09-25, main device (2560x1080) | 'home menu not detected'; the map picker could not run | the 2560x1080 sheet predated the Continue button (Skirmish moved from y 490 to 435) and had none of the picker / menu points; four checks in the code were 2880x1800 literals scaled by SW/SH (the lobby minimap, its corner, the Yes/No dialog, the snapshot crop) | sheet re-measured live; every point from the sheet; `test_no_screen_coordinates_scaled_in_code`; `menu_post_probe` / `postmatch_quit` on 2560x1080 derived, not measured |
| I18 | 2026-09-25, run 14 | `ABORT: mouse in top-left corner` mid-cap | the owner used the desktop during the cap; the abort gesture fires on any pass through the corner | open: tests run only on the dedicated device (owner 2026-09-25); on a shared desktop the gesture needs a hold time |
| I19 | 2026-09-25, run 14 | Istanbul AIs with 0 / 3 / 1 docks and 0 / 3 / 3 warships at 29:00 | 12-15 dock plans per AI failed with 'placement failed' (the stock dock builder since `936ce99a`); the same class as I15 (no buildable shore near the plan's points) | open: `AIDOCKFAIL` echo (the plan's points and gNavyVec) for the next run |
| I20 | 2026-09-25, the owner's test + editor | forward bases on the AI's OWN island; the owner saw NO markers in the editor, then (after the enemy-beach fix) markers for one team only | `4d43d31f` placed a marker for every player on BOTH beaches (the AI picks by distance, which does not tell the shore on a water map) and only in the repo map - the editor copy `000_istanbul.xs` (FILE LAW: byte-identical) never got it; an agent then reported the target as right without checking the shore; the marker had no `PlaceAnywhere` (0 m radius on a beach point that can be cliff) | one marker per player on the ENEMY beach, repo + editor copy hash-identical, `PlaceAnywhere`, the marker test; the markers' sight in the editor is the gate (`2026-09-25-istanbul-handover.md`); the AI's distance rule is open |
| I21 | 2026-09-25, test device | **INCIDENT (owner): about 2 hours and $100+ of Scenario Editor work before a single AI test** | the agent ran the handover's Step 1b / Step 2 editor gates literally (London / Paris / Istanbul generations, saves, marker censuses, a gun-socket A/B) although the owner wanted the AI tested; 'obsessive' meant automated AI-dependency tests, not editor spawn checks | memory `obsessive-means-automated-ai-tests-not-editor`: the AI run goes first; editor work only when the owner names it; briefs must not put editor gates before AI runs |
| I22 | 2026-09-25, run 41 (2880x1800) | after the snapshots the driver printed `batch done`, but the match was still running; no per-player file was archived | `end_match` begins with `probe_ok(home_skirmish)`: in the match, sand sat under the Skirmish pixel and a selected building's dark info panel under the second pixel, so the match passed for the home menu | the agent quit by hand; **open**: require a home-only pixel, or check that `match_cog` is not visible first |
| I23 | 2026-09-25, runs 41-42 | every `AIDOCKFAIL` line reads `plan -1 point0 -1/-1 point1 -1/-1` | `aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, puid)` in the placement handler finds no plan (already gone, or puid is not the plan's building type) | **open**: only gNavyVec is known; the echo needs another plan lookup before it can answer I19 |
