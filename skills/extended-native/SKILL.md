...
```bash
python skills/native-ability-minify/scripts/abilitytool.py slots <Subciv>
```
...
```bash
python skills/aoe3de-bar-archives/scripts/bartool.py cat powers.xml > /tmp/powers.xml
```
...
or a text-mode write strips every CR. These byte-preservation instructions apply
to every agent; do not depend on an agent-specific memory being installed.
...
python skills/native-ability-minify/scripts/abilitytool.py verify
```
